...
import resumePDF from "../assets/Varghese_Jojo.pdf";
export function Navbar({ dark, setDark }) {
      ...
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            const activeSection = sectionId.charAt(0).toUpperCase() + sectionId.slice(1);
  ...
  const handleResumeDownload = (e) => {
    ...
    link.href = resumePDF;
    link.download = 'Varghese_Jojo.pdf'; 
  ...
  return (
    <nav className={`fixed top-4 left-1/2 -translate-x-1/2 w-[94%] max-w-6xl z-50 rounded-2xl transition-all duration-500 ${navStyle}`}>
      <div className="flex items-center justify-between px-6 h-16">
        ...
        >
          <span className="bg-linear-to-r from-cyan-400 via-blue-500 to-purple-600 bg-clip-text text-transparent">VJ</span>
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`p-2 rounded-lg transition ${
              dark ? "bg-white/10 text-yellow-300 hover:bg-white/20" : "bg-gray-100 text-cyan-500 hover:bg-gray-200"
            }`}
          >
            {dark ? <SunIcon /> : <MoonIcon />}
          </button>

          {/* RESUME DOWNLOAD BUTTON - UPDATED */}
          <button
            onClick={handleResumeDownload}
            className="hidden md:flex items-center gap-2 px-4 py-2 text-xs font-bold bg-linear-to-r from-cyan-500 to-blue-600 text-white rounded-lg hover:scale-105 transition-all hover:shadow-lg hover:shadow-cyan-500/25"
          >
            <DownloadIcon /> Resume
          </button>
      ...
      {/* MOBILE MENU */}
      {open && (
        <div className={`md:hidden border-t p-4 flex flex-col gap-2 ${dark ? "border-white/10" : "border-gray-200"}`}>
          {NAV_LINKS.map((link) => (
            <button
              key={link}
              onClick={() => go(link)}
